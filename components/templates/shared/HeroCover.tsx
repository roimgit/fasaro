...
export const HeroCover: React.FC<HeroCoverProps> = ({
  ...
  return (
    ...
    >
      ...
      >
        <p className="text-xs uppercase tracking-[0.3em] opacity-75 mb-3">
          Walimatul Ursy / Undangan Pernikahan
        </p>

        <h1
          className={`text-3xl sm:text-4xl font-serif font-light tracking-wide my-4 ${
            themeStyle?.titleClass ?? "text-stone-50"
          }`}
        >
          {groomName} &amp; {brideName}
        </h1>

        <div className="my-6 py-4 px-3 rounded-2xl bg-white/5 border border-white/10">
          <p className="text-xs tracking-wider text-stone-400 mb-1">Kepada Yth. Bapak/Ibu/Saudara/i:</p>
        ...
        <p className="text-xs tracking-widest text-stone-300 mb-6">{formattedDate}</p>
